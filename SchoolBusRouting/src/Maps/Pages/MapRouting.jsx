...
import { MapPin, Navigation, Plus, Trash2, Route, ChevronDown, ChevronRight } from 'lucide-react';
...
export const MapRouting = () => {
  ...
  const calculateRoute = () => {
    ...
    const validPoints = routePoints.filter(point => point && point.trim() !== '');
      ...
      <div className="flex items-center justify-center h-screen bg-gray-100">
        <div className="text-center">
          ...
          <p className="text-sm text-red-500 mt-2">
            ⚠️ Necesitas reemplazar YOUR_API_KEY con tu clave real de Google Maps
          </p>
        </div>
      </div>
    );
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 p-6">
            ...
            <div className="lg:col-span-1 space-y-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div key={index} className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          placeholder={`Punto ${String.fromCharCode(65 + index)} - ${index === 0 ? 'Inicio' : index === routePoints.length - 1 ? 'Final' : 'Parada'}`}
                        />
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full mt-6 bg-blue-600 text-white p-3 rounded-md hover:bg-blue-700 disabled:bg-gray-400 transition-colors flex items-center justify-center"
                >
                  {loading ? (
                    <>
                  ...
                  <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-md">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">{routeInfo.totalDistance}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-green-700">Tiempo estimado:</span>
                        <span className="font-semibold">{routeInfo.totalDuration}</span>
                      </div>
                    </div>
                    
                    <div className="mt-4">
                      <h5 className="font-semibold text-green-800 mb-2">Segmentos de Ruta:</h5>
                      <div className="space-y-3">
                            ...
                            {expandedSegments[index] && (
                              <div className="p-3">
                                <div className="text-xs text-gray-500 mb-2">
                                  <div><strong>Desde:</strong> {segment.startAddress}</div>
                                  <div><strong>Hasta:</strong> {segment.endAddress}</div>
                                </div>
                                
                                <div className="space-y-2 max-h-60 overflow-y-auto">
                                  <h6 className="font-semibold text-gray-700 text-sm">Instrucciones paso a paso:</h6>
                                  {segment.instructions.map((instruction, stepIndex) => (
                                    <div key={stepIndex} className="flex items-start space-x-2 p-2 bg-gray-50 rounded text-xs">
                                      ...
                                      <div className="flex-1">
                                        <div className="flex items-center space-x-1 mb-1">
                                          <span className="text-lg">{getManeuverIcon(instruction.maneuver)}</span>
                                          <span className="text-gray-600">{instruction.distance} • {instruction.duration}</span>
                                        </div>
                                        <div className="text-gray-800">{instruction.instruction}</div>
                                      </div>
                                    </div>
                                  ))}
                                </div>
            ...
            <div className="lg:col-span-2">
              <div className="bg-gray-200 rounded-lg overflow-hidden" style={{ height: '600px' }}>
